-- gen_random_bytes lives in the `extensions` schema (pgcrypto) and is therefore
-- unreachable under `set search_path = ''`. gen_random_uuid() is core Postgres
-- (pg_catalog), so it resolves regardless of search_path.
...
begin
  ...
  v_ref := 'OB-' || upper(substr(replace(gen_random_uuid()::text, '-', ''), 1, 8));
